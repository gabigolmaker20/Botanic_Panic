...
const Products = () => {
  ...
  const [productoParaConfirmarEliminacion, setProductoParaConfirmarEliminacion] = useState(null);
  ...
  const handleSubmitProducto = (e) => {
    ...
    let esValido = true;
    const errores = [];
    if (!imageSrc && !productoAEditar?.imageSrc) { // Si no hay imagen nueva Y no estamos editando una con imagen
  ...
  const confirmarEliminacionProducto = () => {
    if (productoParaConfirmarEliminacion) {
      setProducts(prevProducts => prevProducts.filter(p => p.id !== productoParaConfirmarEliminacion.id));
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
              ...
              <form onSubmit={handleSubmitProducto} className="space-y-4 px-4 sm:px-6">
                <div>
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed border-gray-300 rounded-lg w-full h-48 sm:h-64 flex items-center justify-center ${!isUploading ? 'cursor-pointer bg-gray-50 hover:bg-gray-100' : 'bg-gray-200 cursor-not-allowed'} transition-all`}
                  >
                    {isUploading ? (
                ...
                <div>
                  ...
                  <input id="nombreProducto" style={{ width: "100%", marginBottom: "10px" }} type="text"
                    ...
                    placeholder="Ej: Menta" value={nombre} onChange={(e) => setNombre(e.target.value)} />
                </div>
                <div>
                  <label htmlFor="categoriaProducto" className="block mb-1.5 font-medium text-emerald-600">Categoría</label>
                  <input id="categoriaProducto" style={{ width: "100%", marginBottom: "10px" }} type="text"
                    className=" border border-gray-300 rounded-md px-3 py-2 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors duration-200 placeholder-gray-400"
                    placeholder="Ej: Aromáticas, Interior, Exterior..." value={categoria} onChange={(e) => setCategoria(e.target.value)} />
                </div>
                <div>
                  <label htmlFor="descripcionProducto" className="block mb-1.5 font-medium text-emerald-600">Descripción</label>
                  <textarea id="descripcionProducto" style={{ width: "100%", marginBottom: "10px" }}
                    className=" border border-gray-300 rounded-md px-3 py-2 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors duration-200 placeholder-gray-400"
                    rows="3" placeholder="Detalles sobre la planta, cuidados, etc." value={descripcion} onChange={(e) => setDescripcion(e.target.value)}></textarea>
                </div>
                <div>
                  <label htmlFor="precioProducto" className="block mb-1.5 font-medium text-emerald-600">Precio</label>
                  <input id="precioProducto" style={{ width: "100%", marginBottom: "10px" }} type="number" step="0.01"
                    className=" border border-gray-300 rounded-md px-3 py-2 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors duration-200 placeholder-gray-400"
                ...
                <div>
                  ...
                  <input id="stockProducto" style={{ width: "100%" }} type="number"
                    className=" border border-gray-300 rounded-md px-3 py-2 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors duration-200 placeholder-gray-400"
                    placeholder="0" value={stock} onChange={(e) => setStock(e.target.value)} />
                </div>
                <div className="flex justify-end pt-4">
                  <button
                    style={{ marginBottom: "15px", marginRight: "0px" }} type="submit" disabled={isUploading}
                    className={`rounded py-2 bg-[#029a67] text-white px-5 py-2.5 rounded-md font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-white transition-colors duration-300 ease-in-out ${isUploading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-emerald-700'}`}
                  >
                    {isUploading ? "Subiendo..." : (productoAEditar ? "Actualizar Producto" : "Guardar Producto")}
                  </button>
                </div>
              </form>
            </div>
        ...
        {mostrarModalEliminar && productoParaConfirmarEliminacion && (
          <div  className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
            <div style={{background:"rgb(243, 245, 235)"}}  className=" rounded-lg shadow-xl p-6 sm:p-8 w-full max-w-md">
              <h3 style={{margin:"10px"}} className="text-xl font-semibold mb-4 text-gray-800">Confirmar Eliminación</h3>
              <p style={{margin:"10px"}} className="text-gray-600 mb-6">
                ¿Estás seguro de que deseas eliminar el producto "<strong>{productoParaConfirmarEliminacion.id&&productoParaConfirmarEliminacion.name}</strong>"? Esta acción no se puede deshacer.
              </p>
              <div style={{background:"rgb(243, 245, 235)"}} className="flex justify-end space-x-3">
                <button style={{marginBottom:"10px",marginRight:"10px"}} onClick={cerrarModalEliminar} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors">Cancelar</button>
                <button style={{marginBottom:"10px",marginRight:"10px"}} onClick={confirmarEliminacionProducto} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors">Eliminar</button>
              </div>
            </div>
          </div>
        )}
              ...
              <div className="mt-4 flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm text-gray-700">
                    <h6 href={product.href} className="hover:underline">{product.name}</h6>
                  </h3>
                  <p className="text-sm font-medium text-gray-900">{product.price}</p>
                </div>
                <div className="mt-1 flex justify-center">
                  <button className="bg-[#091a04] text-amber-50 w-full rounded py-2 font-semibold group-hover:scale-95 transition-all duration-300 ease-in-out">Añadir al carrito</button>
                </div>
